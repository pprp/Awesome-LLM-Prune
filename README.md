...
- Efficient Transformer-Based Piano Transcription With Sparse Attention   Mechanisms
    ...
    - Code: Not available
    - Pub: Arxiv 2025
    - Summary: This paper introduces a computationally efficient Transformer architecture for piano transcription using sparse attention mechanisms, including sliding-window self-attention and a hybrid global-local cross-attention. By incorporating these sparse attention techniques and a hierarchical pooling strategy, the proposed model significantly reduces computational cost and memory usage while maintaining transcription accuracy comparable to full-attention models, enabling the processing of longer audio contexts. #SparseAttention #PianoTranscription #EfficientTransformer #MusicAI
    - 摘要: 这篇论文提出了一种基于稀疏注意力机制的高效Transformer钢琴转录模型，旨在解决传统Transformer模型在处理长音乐片段时计算复杂度过高的问题。通过引入滑动窗口自注意力机制和混合全局-局部交叉注意力机制，并结合分层池化策略，该模型在保持转录性能的同时，显著降低了计算成本和内存占用，实现了更快的推理速度，并支持更长的音频上下文训练。#钢琴转录 #Transformer #稀疏注意力 #高效模型 #音乐信息处理
