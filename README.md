...
- (P)rior(D)yna(F)low: A Priori Dynamic Workflow Construction via   Multi-Agent Collaboration
    ...
    - Code: Not available
    - Pub: Arxiv 2025
    - Summary: This paper introduces Prior DynaFlow, a novel framework for automated LLM workflow construction that combines Q-table learning to leverage historical experience with a priori decision-making based on task progress. By enabling agents to proactively select the most suitable workflow structure and incorporating efficiency mechanisms like cold-start initialization and pruning, Prior DynaFlow achieves significant improvements in task-solving performance while substantially reducing workflow construction and inference costs compared to existing methods. #LLMWorkflows #MultiAgentSystems #DynamicWorkflow #Efficiency #QLearning
    - 摘要: 该论文提出了一种名为Prior DynaFlow的先验动态工作流构建框架，旨在提升LLM在复杂任务中的表现。该框架通过多智能体协作，结合Q-table学习优化决策空间，并根据任务进度进行先验决策，动态选择更合适的工作流结构。实验结果表明，该方法在提升性能的同时，显著降低了工作流构建和推理成本。#工作流构建 #多智能体协作 #动态调整 #LLM优化
