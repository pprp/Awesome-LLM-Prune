...
- A Sentinel-3 foundation model for ocean colour
    ...
    - Code: Not available
    - Pub: Arxiv 2025
    - Summary: This paper introduces a Sentinel-3 foundation model based on the Prithvi-EO Vision Transformer, pre-trained for reconstructing data from the Ocean and Land Colour Instrument (OLCI). The model demonstrates strong performance when fine-tuned for chlorophyll concentration quantification and refining ocean primary production estimates, showcasing its utility for marine monitoring and leveraging limited labeled data. #FoundationModels #OceanColour #RemoteSensing #VisionTransformer
    - 摘要: 该研究提出了一种基于Prithvi-EO Vision Transformer架构的Sentinel-3海洋颜色基础模型，通过自监督学习重建OLCI数据进行预训练。该模型在叶绿素浓度估计和海洋初级生产力评估等下游任务中表现出色，尤其是在利用少量高质量标注数据和捕捉海洋颜色空间细节方面优于现有模型，证明了其在海洋监测中的潜力。#海洋颜色 #基础模型 #自监督学习 #遥感 #Vision Transformer
