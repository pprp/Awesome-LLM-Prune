...
- Efficient Transformer-Based Piano Transcription With Sparse Attention   Mechanisms
    ...
    - Code: Not available
    - Pub: Arxiv 2025
    - Summary: This paper introduces an efficient Transformer architecture for piano transcription using sparse attention mechanisms, including sliding-window self-attention and a hybrid global-local cross-attention, along with hierarchical pooling. By reducing computational cost and memory usage, the proposed model achieves comparable transcription performance to full-attention models while enabling training with longer audio contexts. The work demonstrates the effectiveness of sparse attention for building efficient and high-performance music transcription systems. #SparseAttention #Transformer #PianoTranscription #MusicAI #EfficientAI
    - 摘要: 该论文提出了一种基于稀疏注意力机制的高效Transformer钢琴转录模型，旨在降低计算成本并处理更长的音乐片段。通过引入滑动窗口自注意力机制和混合全局-局部交叉注意力机制，并结合分层池化策略，该模型在保持转录性能的同时，显著减少了计算量和内存使用，实现了更快的推理速度。#钢琴转录 #Transformer #稀疏注意力 #计算效率 #音乐信息处理
